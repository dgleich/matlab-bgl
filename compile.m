...
if octave, mexflags = [mexflags ' -DMX_API_VER=0x07030000 -DMWINDEX_MAX=2147483647 ']; end

if verbose, mexflags = [mexflags ' -v ']; end
if debug, mexflags = [mexflags ' -D_DEBUG -g ']; 
elseif ~octave, mexflags = [mexflags ' -O '];
end
...
elseif isunix
    % 
    if ~octave, mexflags = [mexflags ' CFLAGS="\$CFLAGS -Wall" ']; end
    if solaris
    else
